...
public class FileManageActionsBean implements FileManageActions {
	...
	public final static int[] exeMagicHeader = new int[]{ 0x4d, 0x5a, 0x90, 0x00, 0x3, 0x00, 0x00, 0x00 };
    ...
    public void processUpload(FileUploadEvent uploadEvent) {
        try {
            if (fileUploadHolder != null) {
                ...
                if(blob==null){// code change to check .exe file
                	 facesMessages.add(StatusMessage.Severity.ERROR, ".exe file not allowed");
                }else{
    ...
    public static FileBlob getBlob(FileUploadEvent uploadEvent) throws IOException {
        ...
        try (InputStream in = uploadedFile.getInputStream()) {
        	 if(isExe(in)){// code change to check .exe file
                 return null;
        	 }
...
/*
 * method added by Santosh Yadav to check .exe file
 */
    public static boolean isExe(InputStream ins) {
        try {
            for (int i = 0; i < 8; i++) {
