...
import org.nuxeo.custom.logger.CustomLogService;
...
public class NuxeoAuthenticationFilter implements Filter {
    ...
    protected Principal doAuthenticate(CachableUserIdentificationInfo cachableUserIdent,
        ...
        try {
            ...
            // code change to set lastLogin time by calling getLastLogin of CustomLogService  
            NuxeoPrincipalImpl nuxeoPrincipalImpl=(NuxeoPrincipalImpl) loginContext.getSubject().getPrincipals().toArray()[0];
            CustomLogService reader = Framework.getService(CustomLogService.class);
            nuxeoPrincipalImpl.setLastLogin(reader.getLastLogin(nuxeoPrincipalImpl.getName()));
           // code change to set lastLogin time by calling getLastLogin of CustomLogService
            
//            Principal principal = (Principal) loginContext.getSubject().getPrincipals().toArray()[0];
            cachableUserIdent.setPrincipal(nuxeoPrincipalImpl);
            ...
            cachableUserIdent.getUserInfo().setUserName(nuxeoPrincipalImpl.getName());
            ...
            if (principal == null) {
                ...
                if (cachableUserIdent == null || cachableUserIdent.getUserInfo() == null) {
                    ...
                    if (userIdent != null && userIdent.containsValidIdentity()) {
                    	...
                    	// code change to get user already logged in with same ID
                    	Collection<SessionInfo> trackedSessions = NuxeoHttpSessionMonitor.instance().getTrackedSessions();
                        for (SessionInfo sessionInfo : trackedSessions) {
	                         if(sessionInfo.getLoginName()!=null && sessionInfo.getLoginName().equals(userIdent.getUserName()) ){
	                        	 httpRequest.setAttribute(LOGIN_ERROR,ALREADY_LOGGEDIN);
	                             boolean res = handleLoginPrompt(httpRequest, httpResponse);
	                             if (res) {
	                                    return;
	                             }
